...
def load_and_validate_data(market_data_path, tickers_ref_path):
    ...
    try:
        ...
        raise FileNotFoundError(f"Could not find market data file: {market_data_path}")
        
    try:
        tickers_ref_df = pd.read_csv(tickers_ref_path)
    except FileNotFoundError:
        raise FileNotFoundError(f"Could not find tickers reference file: {tickers_ref_path}")
